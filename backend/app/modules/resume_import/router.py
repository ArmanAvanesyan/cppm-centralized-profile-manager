...
@router.post("/{resume_id}/extract", status_code=status.HTTP_202_ACCEPTED, response_model=schema.JobAcceptedResponse)
def extract_resume(
    resume_id: str,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
...
@router.post("/{resume_id}/parse", status_code=status.HTTP_202_ACCEPTED, response_model=schema.JobAcceptedResponse)
def parse_resume(
    resume_id: str,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
